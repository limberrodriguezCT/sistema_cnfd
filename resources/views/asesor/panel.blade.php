...
@section('content')
            ...
            <form action="{{ route('asesor.store_asignacion') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-6">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <select name="grupo_id" required class="w-full text-sm p-3 border border-gray-300 rounded-xl bg-gray-50 focus:ring-2 focus:ring-purple-500">
                            <option value="">Seleccione Grupo</option>
                            @if(count($grupos) > 0)
                                @foreach($grupos as $g)
                                    <option value="{{ $g->id }}">{{ strtoupper($g->codigo_grupo) }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="md:col-span-1">
                        ...
                        <select name="modulo_id" required class="w-full text-sm p-3 border border-gray-300 rounded-xl bg-gray-50 focus:ring-2 focus:ring-purple-500">
                        ...
                        @if(count($asignaciones) > 0)
                            @foreach($asignaciones as $asig)
                                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors filtro-fila {{ $asig->trashed() ? 'opacity-50' : '' }}">
                                    ...
                                    <td class="py-4 px-4"><span class="bg-purple-100 text-purple-800 font-bold px-2 py-1 rounded">{{ strtoupper($asig->grupo->codigo_grupo ?? 'N/A') }}</span></td>
    ...
    <div id="tab-interesados" class="tab-content hidden space-y-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 border-b bg-emerald-50 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <input type="text" id="busc-prospectos" onkeyup="filtrarTabla('busc-prospectos', 'tabla-prospectos')" placeholder="Buscar por nombre, correo..." class="px-3 py-2 border border-emerald-200 rounded-lg text-sm w-full md:w-64 shadow-sm focus:ring-2 focus:ring-emerald-500">
            </div>
            <div class="overflow-x-auto p-4">
                <table class="w-full text-left text-sm border-collapse" id="tabla-prospectos">
                    <thead>
                        <tr class="text-gray-500 border-b border-gray-200 uppercase text-xs tracking-wider">
                            <th class="py-3 px-4 font-bold">Fecha de Registro</th>
                            <th class="py-3 px-4 font-bold">Datos del Interesado</th>
                            <th class="py-3 px-4 font-bold">Modalidad Solicitada</th>
                            <th class="py-3 px-4 font-bold">Oferta Formativa</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if(count($interesados) > 0)
                            @foreach($interesados as $prospecto)
                                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors filtro-fila">
                                    <td class="py-4 px-4 font-bold text-gray-500 whitespace-nowrap">{{ $prospecto->created_at->format('d/m/Y h:i A') }}</td>
                                    <td class="py-4 px-4">
                                        <p class="font-black text-gray-800 text-base">{{ $prospecto->nombre }}</p>
                                        <div class="flex items-center gap-3 mt-1 text-xs font-semibold">
                                            <span class="text-blue-600 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> {{ $prospecto->correo }}</span>
                                            <span class="text-emerald-600 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg> {{ $prospecto->telefono }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <span class="bg-gray-100 text-gray-700 font-bold px-3 py-1 rounded-full text-xs uppercase border border-gray-200">{{ $prospecto->modalidad }}</span>
                                    </td>
                                    <td class="py-4 px-4 font-bold text-emerald-900">{{ $prospecto->carrera->nombre ?? 'N/A' }}</td>
                                </tr>
                            @endforeach
                        @else
    ...
    <!-- MODAL DE EXPORTACIÓN GLOBAL (FILTRO POR FECHA) -->
    ...
    <!-- MODAL DE EXPORTACIÓN INDIVIDUAL (ASESOR) -->
...
<script>
    ...
    function abrirModalExportar(asignacionId) {
        document.getElementById('formExportarConsolidado').action = `/asesor/exportar-reporte/${asignacionId}`;
        document.getElementById('formExportarSemaforo').action = `/asesor/exportar-reporte/${asignacionId}`;
